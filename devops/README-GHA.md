...
2. Go to your repository at [fferreir/pos](https://github.com/fferreir/pos).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `posepidemiovps.fmvz.usp.br` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | posepidemiovps.fmvz.usp.br | The hostname or IP address of your Docker Swarm manager. |
...
| Name     | Value |
|----------|-------|
| LIVE_ENV | posepidemiovps.fmvz.usp.br |
...
- [Backend Tests Workflow](https://github.com/fferreir/pos/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/fferreir/pos/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/fferreir/pos-backend`) and Frontend (`ghcr.io/fferreir/pos-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of posepidemiovps.fmvz.usp.br](https://github.com/fferreir/pos/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/posepidemiovps.fmvz.usp.br.yml` stack.
