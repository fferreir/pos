...
PROJECT_NAME=pos
STACK_NAME=posepidemiovps-fmvz-usp-br
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://pos.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t fferreir/pos-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t fferreir/pos-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name pos-frontend-acceptance --link pos-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d fferreir/pos-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name pos-backend-acceptance -d fferreir/pos-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop pos-frontend-acceptance
	@docker stop pos-backend-acceptance
